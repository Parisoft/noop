...
class Expressions {
	...
	def isOnMemberSelectionOrReference(Expression expression) {
		...
		container !== null &&
			(container instanceof MemberSelect || container instanceof MemberRef ||
						...
						NewInstance: {
							...
							if (expression.isOnMemberSelectionOrReference) {
								if (expression.dimension.isEmpty && expression.type.isNonPrimitive) {
									...
									chunks +=
										ctx.computeTmp(expression.nameOfTmpArray(ctx.container), expression.fullSizeOf)
								}
							}
				...
				private def compileBinary(Operation binaryOperation, Expression left, Expression right,
					CompileContext ctx) '''
			...
			mode = Mode::OPERATE
				...
				private def compileMultiplication(Operation operation, Expression left, Expression right,
					CompileContext ctx) {
					try {
